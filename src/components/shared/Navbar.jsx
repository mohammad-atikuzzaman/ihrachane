  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-100 ${
        isScrolled ? "bg-white text-gray-800 shadow-md py-2 text-" : "bg-transparent text-white py-4"
      }`}
    >
          ...
          <div className="hidden md:flex items-center space-x-6">
            {menus?.map((menu, index) => (
              <Link
                key={index}
                href={menu.url}
                className=" hover:text-orange-600 px-3 py-2 rounded-md text-lg font-medium transition-colors duration-200"
              >
                {menu.path}
              </Link>
            ))}
